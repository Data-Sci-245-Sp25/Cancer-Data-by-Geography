...
withfips <- rawData|>
  mutate(padtract= str_pad(ddrestract, width = 4, pad = "0"))

# create padtractsuf with 2 digits
withfips <- withfips|>
  mutate(padtractsuf = str_pad(ddrestractsuf, width = 2, pad = "0"))

#concactenate into fipscode
withfips <- withfips|>
  mutate(fipscode = paste("41051", padtract, padtractsuf, sep = ""))

#remove padded columns
withfips <- withfips|>
  select(-padtract, -padtractsuf)
...
tidydata <- cancerOnlyData |>
  left_join(puma, by = c("drescountyfips" = "COUNTYFP", "ddrestract" = "TRACTCE"))
